...
class SaleOrder(models.Model):
    ...
    def create(self, vals):
        for line in vals.get('order_line'):
            if 'discount' not in line[2]:
                product_obj = self.env['product.product'].browse(line[2].get('product_id'))
                ...
                product_uom = self.env['uom.uom'].browse(line[2].get('product_uom'))
                product_uom_qty = line[2].get('product_uom_qty')
